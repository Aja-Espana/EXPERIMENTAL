...
{
    // 1. Calculate the smoothing factor (creates the soft edge)
    // We keep this logic exactly as Unity intended to avoid the "square" look.
    float2 distanceAttenuationFloat = float2(distanceAttenuation);
    half factor = half(distanceSqr * distanceAttenuationFloat.x);
    half smoothFactor = saturate(half(1.0) - factor * factor);
    smoothFactor = smoothFactor * smoothFactor;

    // 2. Return ONLY the smooth factor.
    // By removing 'rcp(distanceSqr)', the light does not get dimmer with distance.
    // It stays 100% bright until it hits the soft edge defined by the Light's Range.
    return smoothFactor; 
}    

half AngleAttenuation(half3 spotDirection, half3 lightDirection, half2 spotAttenuation)
{
    // Compute the cosine of the angle between spotlight direction and light direction
    half SdotL = dot(spotDirection, lightDirection);

    // Standard calculation for interpolation factor
    // spotAttenuation.x = 1 / (cosInner - cosOuter)
    // spotAttenuation.y = -cosOuter * spotAttenuation.x
    half atten = SdotL * spotAttenuation.x + spotAttenuation.y;

    // ORIGINAL SOFT CODE:
    // atten = saturate(atten);
    // return atten * atten; // This squaring creates the smooth gradient

    // NEW HARD CODE:
    // If atten >= 1.0 (inside inner cone), return 1.0
    // If atten <= 0.0 (outside outer cone), return 0.0
    // To make it HARD (no soft edge), we simply threshold it.
    
    // Option A: Completely Hard Cutoff (Pixelated edge)
    return (atten > 1.0) ? 1.0 : 0.0; 
    
    // Option B: Hard Inner, Sharp Outer (Linear step, no smooth curve)
