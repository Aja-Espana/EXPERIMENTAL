            ...
            IPasteRecievier
    {
        ...
        public bool IsHit(Vector2 mousePosition)
        {
            return layout.body.Contains(mousePosition)
                && !layout.inConnectionHitbox.Contains(mousePosition)
                && !layout.outConnectionHitbox.Contains(mousePosition);
        }
        ...
        public override void OnGUI()
        {
            ...
            HoverType hoverType = GetHoverType(mousePos);

            // Draw Node Body Background and Outline first
            ...
            // In-Connection Rendering drawn OVER the rest of the node
            if (
            ...
            {
                bool isInHighlighted = hoverType == HoverType.InConnection;
                Color inColor = isInHighlighted ? Prefs.highlightColor : (Styles.WindowBackground * tint);
                GUI.color = inColor;
                SchemaGUI.DrawRoundedBox(layout.inConnection, inColor, new Vector4(8, 8, 0, 0));
            }

            // Out-Connection Rendering drawn OVER the rest of the node
            if (
                node.connectionDescriptor == Node.ConnectionDescriptor.OnlyOutConnection
                || node.connectionDescriptor == Node.ConnectionDescriptor.Both
            )
            {
                bool isOutHighlighted = hoverType == HoverType.OutConnection;
                Color outColor = isOutHighlighted ? Prefs.highlightColor : (Styles.WindowBackground * tint);
                GUI.color = outColor;
                SchemaGUI.DrawRoundedBox(layout.outConnection, outColor, new Vector4(0, 0, 8, 8));
            }
        ...
        private void DoEvents()
        {
            ...
            switch (e.rawType)
            {
                case EventType.MouseDown when e.button == 0:
                    ...
                    if (layout.inConnectionHitbox.Contains(e.mousePosition))
                    {
                        Deselect();
                    ...
                    else if (layout.outConnectionHitbox.Contains(e.mousePosition) && node.CanHaveChildren())
                    {
                        Deselect();
                ...
                case EventType.MouseUp:
                    ...
                    // Check against floatingConnection.from instead of 'this'
                    if (hovered != null && hovered != floatingConnection.from)
                    {
                        ...
                        {
                            // Dropped on another node's body (invalid port target)
                    ...
                    {
                        // Dropped into empty canvas space -> Open "Add New Node" search
                        NodeComponent sourceNode = floatingConnection.from;

                        floatingConnection.Delete();
                        Destroy(floatingConnection);

                        if (sourceNode != null)
                            DoConnectionDrop(sourceNode, mousePositionGrid);
                    }
                    else
                    {
                        // Dropped back onto the source node itself
                        floatingConnection.Delete();
                        Destroy(floatingConnection);
                    }
                ...
                case EventType.ScrollWheel when floatingConnection != null:
                    e.Use();
                    break;
                case EventType.MouseDrag when e.button == 0 && isSelected && canvas.selectionBoxComponent.hidden && floatingConnection == null:
        ...
        {
            if (layout.inConnectionHitbox.Contains(mousePosition))
                return HoverType.InConnection;

            if (layout.outConnectionHitbox.Contains(mousePosition))
                return HoverType.OutConnection;

            // If dragging a connection wire and dropping on the node body, snap to the appropriate connection port
            if (floatingConnection != null && layout.body.Contains(mousePosition))
            {
                if (floatingConnection.from != null && node.CanHaveParent())
                    return HoverType.InConnection;

                if (floatingConnection.to != null && node.CanHaveChildren())
                    return HoverType.OutConnection;
            }
        ...
        public class NodeComponentLayout
        {
            ...
            public Rect inConnectionHitbox { get; set; }
            public Rect outConnectionHitbox { get; set; }
            ...
            public void Update()
            {
                ...
                inConnectionHitbox = inConnection.Pad(-10);
                outConnectionHitbox = outConnection.Pad(-10);
