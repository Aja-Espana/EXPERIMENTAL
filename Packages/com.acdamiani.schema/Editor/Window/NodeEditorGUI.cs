...
namespace SchemaEditor
{
    public partial class NodeEditor
    {
        ...
        {
            // Clean up blackboard editor when target changes or target object is destroyed
            if (blackboardEditor != null && (blackboardEditor.target != target.blackboard || !blackboardEditor.target))
            {
                DestroyImmediate(blackboardEditor);
                blackboardEditor = null;
            }

            if (blackboardEditor == null && target != null && target.blackboard != null)
            {
                blackboardEditor = Editor.CreateEditor(target.blackboard, typeof(BlackboardEditor));
            }

            // Safely destroy existing editors if any target inside them has been destroyed
            if (editor != null && editor.targets.Any(x => x == null))
            {
                DestroyImmediate(editor);
                editor = null;
            }

            bool isNodeTarget = editor != null && editor.targets.All(x => x is Node);
            bool isConditionalTarget = editor != null && editor.targets.All(x => x is Conditional);

            if (defaultNodeEditor != null && (defaultNodeEditor.targets.Any(x => x == null) || !isNodeTarget))
            {
                DestroyImmediate(defaultNodeEditor);
                defaultNodeEditor = null;
            }

            if (defaultConditionalEditor != null && (defaultConditionalEditor.targets.Any(x => x == null) || !isConditionalTarget))
            {
                DestroyImmediate(defaultConditionalEditor);
                defaultConditionalEditor = null;
            }

            // Fetch active editable targets from canvas
            List<Object> targets = canvas != null
                ? canvas.selected
                    .OfType<IEditable>()
            ...
            // Reset and exit if multi-selecting different types or selecting nothing
            if (distinctTypes.Count != 1)
            {
                if (editor != null) { DestroyImmediate(editor); editor = null; }
                if (defaultNodeEditor != null) { DestroyImmediate(defaultNodeEditor); defaultNodeEditor = null; }
                if (defaultConditionalEditor != null) { DestroyImmediate(defaultConditionalEditor); defaultConditionalEditor = null; }
                return;
            }

            // Create or update main editor
            if (editor == null)
            {
                if (targets.Count > 0)
            ...
            // Create or update specialized sub-editors
        ...
        private void RebuildComponentTree()
        {
            ...
            var validNodes = target.nodes.Where(x => x != null).ToList();
            ...
            IEnumerable<Node> nodesWithoutComponent = validNodes
            ...
            IEnumerable<NodeComponent> componentsWithoutNode = nodeComponents
                .Where(x => !validNodes.Contains(x.node));

            foreach (NodeComponent nodeComponent in componentsWithoutNode)
                GUIComponent.Destroy(nodeComponent);

            IEnumerable<Conditional> conditionalsWithoutComponent = validNodes
            ...
            IEnumerable<ConditionalComponent> componentsWithoutConditional = conditionalComponents
                .Where(x => !validNodes.Select(x => x.conditionals).Where(x => x != null).SelectMany(x => x).Contains(x.conditional));
            ...
            IEnumerable<Tuple<Node, Node>> connectionsWithoutComponent = validNodes
                .Where(x => x.children != null)
                .Select(x => x.children.Where(y => y != null).Select(y => new Tuple<Node, Node>(y.parent, y)))
                .SelectMany(x => x)
